using System;
using System.Collections.Generic;

namespace EasyLab.Models;

public partial class ExportModelli
{
    public int Id { get; set; }

    public string? Ean13 { get; set; }

    public string? NewSku { get; set; }

    public string? SkuSizeXIlWeb { get; set; }

    public string? Mod { get; set; }

    public string? Tess { get; set; }

    public string? CodCol { get; set; }

    public byte? Scal { get; set; }

    public string? Nome { get; set; }

    public string? DescColBst { get; set; }

    public string? DescColNew { get; set; }

    public string? Tg { get; set; }

    public string? CategoryCode { get; set; }

    public string? Category { get; set; }

    public string? MicroCategoryCode { get; set; }

    public string? MicroCategory { get; set; }

    public string? KitCode { get; set; }

    public string? Kit { get; set; }

    public string? StagCode { get; set; }

    public string? Season { get; set; }

    public byte? AnnoCode { get; set; }

    public string? Anno { get; set; }

    public string? ModelWearsEng { get; set; }

    public string? ModelWearsIt { get; set; }

    public string? Fit { get; set; }

    public string? SkiCollection { get; set; }

    public string? DescriptionShort { get; set; }

    public string? DescriptionEnLong { get; set; }

    public string? DescriptionItLong { get; set; }

    public string? TechFeatures { get; set; }

    public string? SustainableFeatures { get; set; }

    public string? Composition { get; set; }

    public double? Cog { get; set; }

    public double? RetailPriceEuro { get; set; }

    public double? RetailPriceChf { get; set; }

    public double? RetailPriceUsd { get; set; }

    public double? RetailPriceGbp { get; set; }

    public int? Quantity { get; set; }

    public string? InsertDateBst { get; set; }

    public string? DocNumberBst { get; set; }

    public string? DocTypeBst { get; set; }

    public string? CodiceDeposito { get; set; }

    public string? Status { get; set; }

    public string? CoefficienteVolumetrico { get; set; }

    public int? IdStag { get; set; }

    public int? IdMod { get; set; }

    public int? IdModCol { get; set; }

    public int? CkLink { get; set; }

    public int? CkTse { get; set; }

    public int? IdKw { get; set; }

    public string? Mid { get; set; }

    public string? HsCode { get; set; }

    public int? IdSet { get; set; }

    public int? IdSetTg { get; set; }

    public int? TotMag { get; set; }

    public string? CatMercMacroCod { get; set; }

    public string? CatMercMacroDesc { get; set; }

    public string? CatMercCod { get; set; }

    public string? CatMercDesc { get; set; }

    public int? CkEanplm { get; set; }

    public string? Ean13Plm { get; set; }

    public int? CkInEsaurimento { get; set; }

    public int? MagN001 { get; set; }

    public int? MagMl01 { get; set; }

    public int? MagMl02 { get; set; }

    public int? MagMl03 { get; set; }

    public int? MagMl04 { get; set; }

    public int? MagMl05 { get; set; }

    public int? StockN001 { get; set; }

    public int? StockMl01 { get; set; }

    public int? StockMl02 { get; set; }

    public int? StockMl03 { get; set; }

    public int? StockMl04 { get; set; }

    public int? StockMl05 { get; set; }

    public bool? CkMl05SetErrato { get; set; }

    public bool? CkInEsaurimento2 { get; set; }

    public int? TotMagModCol { get; set; }

    public string? Nota { get; set; }

    public int? CkGenerico { get; set; }

    public int? CkEantse { get; set; }

    public int? InvM5 { get; set; }

    public int? InvM3 { get; set; }

    public int? InvTot { get; set; }
}
